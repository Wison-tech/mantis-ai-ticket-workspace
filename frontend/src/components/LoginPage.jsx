...
import { LayoutDashboard, Lock, User, Loader2, Sparkles } from 'lucide-react';
...
const LoginPage = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const response = await ticketApi.login(username, password);
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 p-6">
      <div className="w-full max-w-md animate-fade-in">
        ...
        <p className="text-center text-slate-600 text-[10px] font-bold uppercase tracking-widest mt-10">
          Secure AI-Powered Infrastructure
        </p>
      </div>
    </div>
  );
